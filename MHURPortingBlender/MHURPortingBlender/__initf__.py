...
def import_material(target_slot: bpy.types.MaterialSlot, material_data, mat_type):
    ...
    if bpy.data.node_groups.get(parent_name) != None:
        ...
        imported_shader_node = MAIN_SHADER.nodes.new(type="ShaderNodeGroup")
        ...
        MAIN_SHADER.outputs.new("NodeSocketShader", "BSDF")
        ...
        for output in group_inputs.outputs:
            if imported_shader_node.inputs.get(output.name) != None:
               MAIN_SHADER.links.new(output, imported_shader_node.node_tree.inputs.get(output.name))
        MAIN_SHADER.links.new(imported_shader_node.outputs[0], MAIN_SHADER.inputs[0])
    ...
    if not parent_node_exists:
        for output in group_inputs.outputs:
            if imported_shader_node.inputs.get(output.name) != None:
                MAIN_SHADER.links.new(output, imported_shader_node.inputs.get(output.name))
